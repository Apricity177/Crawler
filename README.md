...
千里马使用公开搜索接口采集，不需要在凭据文件中配置账号密码。

采集时只保留仍可参与的招标公告和采购公告；中标候选人、中标/成交结果、合同、更正及终止公告不会作为新商机保存。正文采购需求不足且存在附件时，程序会按需解析 PDF、DOCX、XLSX、CSV 或 TXT，最多读取 3 个附件。
...
- 产品匹配采用“模型语义评分 + 公告原文证据 + 本地产品边界规则”复核；高于明确匹配阈值才写入产品列，单独出现银行、金融、GPU、服务器、培训或设备等词不会直接判定为产品匹配。
